...
```
【従来】                          【MCP導入後】
AI App A ──独自実装──→ Tool 1     AI App A ─┐
AI App A ──独自実装──→ Tool 2               ├── MCP ──→ Tool 1
AI App B ──独自実装──→ Tool 1               ├── MCP ──→ Tool 2
AI App B ──独自実装──→ Tool 2     AI App B ─┘         → Tool 3
  (N×M個のカスタム実装)             (統一プロトコル)
```
...
```
┌─────────────────────────────────────────────────────┐
│                 MCP Host（例: Claude Desktop）         │
│                                                       │
│   ┌──────────┐  ┌──────────┐  ┌──────────┐          │
│   │ Client 1 │  │ Client 2 │  │ Client 3 │   [LLM]  │
│   └────┬─────┘  └────┬─────┘  └────┬─────┘          │
│        │              │              │                │
└────────┼──────────────┼──────────────┼────────────────┘
         │              │              │
...
![MCP Sequence Diagram](/blog/assets/img/mcp-sequence-diagram.png)
...
| `Query + Tools` | Our Server → Claude | ユーザーの質問 + ツール定義をLLMに提示 |
| `ToolUse` | Claude → Our Server | LLMが選んだツール名と引数 |
| `CallToolRequest` | Client → Server | 指定ツールを指定引数で実行するリクエスト |
| `CallToolResult` | Server → Client | ツール実行結果 |
| `toolResult` | Client → Our Server | 実行結果をアプリケーション側に返す |
